...
def load_deberta_tokenizer_and_model():
    ...
    #tokenizer = AutoTokenizer.from_pretrained("/checkpoint/ram/tianjian/modernbert-similarity-classifier")
    #model = AutoModelForSequenceClassification.from_pretrained("/checkpoint/ram/tianjian/modernbert-similarity-classifier").to(DEVICE)
    #tokenizer = AutoTokenizer.from_pretrained("/checkpoint/ram/tianjian/similarity-classifier-f168-hf/")
    #model = AutoModelForSequenceClassification.from_pretrained("/checkpoint/ram/tianjian/similarity-classifier-f168-hf/").to(DEVICE)
    model.eval()
...
async def classifier_score(prompt: str, s1: str, s2: str):
    ...
    attention_mask = torch.tensor(
        [1] * len(input_ids), device=DEVICE, dtype=torch.int64
    )
    ...
    outputs = model(input_ids=iids.unsqueeze(0), token_type_ids=tids.unsqueeze(0), attention_mask=attention_mask.unsqueeze(0))
