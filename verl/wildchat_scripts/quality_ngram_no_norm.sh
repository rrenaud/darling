...
PYTHONBUFFERED=1 srun --overlap --nodes 1 --ntasks=1 -w "$head_node" \
    ...
    trainer.experiment_name=quality_ngram_multiplicative_no_norm_b${B}_n${N}_l${L} \
    trainer.n_gpus_per_node=8 \
    trainer.nnodes=1 \
    trainer.save_freq=200 \
    trainer.test_freq=200 \
    trainer.default_local_dir=/checkpoint/ram/tianjian/checkpoints_athene/wildchat10k_quality_ngram_multiplicative_no_norm_b${B}_n${N}_l${L} \
    trainer.validation_data_dir=/checkpoint/ram/tianjian/checkpoints_athene/wildchat10k_quality_ngram_multiplicative_no_norm_b${B}_n${N}_l${L}/rollouts \
